...
Every `image.repository` field and the IAM role ARN in `values-api-gateway.yaml` contain the instructor's account ID (`866259084078`). Replace it with your own.
...
find envs/ -name "*.yaml" -exec sed -i '' 's/866259084078/YOUR_ACCOUNT_ID/g' {} +
```

After this, every `image.repository` will point to your ECR registry, e.g.:
```yaml
image:
  repository: 866259084078.dkr.ecr.us-east-1.amazonaws.com/auth-service
```
...
```yaml
DB_HOST: pharma-dev-postgres.ctiikmkmmsnk.eu-central-1.rds.amazonaws.com
```
...
grep -r "866259084078\|cyrywaguk6v4" envs/
```
...
source:
  ...
  repoURL: https://github.com/kelvinSeamount/zen-gitops.git
  path: helm-charts
  helm:
    valueFiles:
